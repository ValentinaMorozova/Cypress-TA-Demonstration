describe('UI tests', () => {
  ...
  it('002.1. Search computers by different prices ranges', () => {
    ...
    cy.get('#advanced-search-block').should('have.attr', 'style', 'display: block;');
    ...
    cy.get('.search-results').get('.item-box').get('.product-title').eq(0).should('contain.text', 'Build your own cheap computer')
  ...
  it('003.1. Add default computer to cart', () => {
    ...
    cy.get('#bar-notification.success').should('be.visible').should('contain.text', 'The product has been added to your shopping cart');
  ...
  it('004. Checkout process with logged in user - all default options', () => {
    ...
    cy.get('.order-completed > .title').should('contain.text', 'Your order has been successfully processed!');
  })
})